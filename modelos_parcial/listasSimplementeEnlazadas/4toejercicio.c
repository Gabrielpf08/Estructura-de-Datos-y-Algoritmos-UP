...
void generar(nodo *h, nodo1 *aux)
{
  while(h->sig!=NULL)
  {
      if(h->num1 + h->num2 + h->num3 > 10)
      {
        aux->num = h->num1 + h->num2 + h->num3; //suma tres numeros

        aux->sig=(nodo1*)malloc(sizeof(nodo1));
        aux=aux->sig;
      }
      h=h->sig;
  }
...
{nodo *aux;
    while(p->sig!=NULL && p->sig->sig!=NULL) //INSERTAR ANTES DE CADA NODO OTRO CON LA MITAD 
    {
...
void eliminar2(nodo* p)
{
    ...
    {
        if(p->sig->num1 * p->sig->num2 * p->sig->num3 < 30) //eLIiminar productoria menor a 30
        {
            aux=p->sig->sig;
            free(p->sig);
            p->sig=aux;
